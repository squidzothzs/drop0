...
export function StoreProvider({ children }) {
  ...
  // one clock for every card countdown — 20 cards ticking their own interval is 20
  // timers to keep in sync for no gain. Stops entirely when nothing is mid-claim.
  const [now, setNow] = useState(() => Date.now())
  const anyExpiring = items.some(i => i.claimExpiresAt)
  useEffect(() => {
  ...
  const value = {
    state: { appState, items, remainingCount, myClaims, heldPieceId, now },
    startClaim,
    confirmClaim,
    releaseClaim,
  }
